/*
 * files.js: Instance methods for working with files from Joyent Manta Storage Service
 *
...
var fs = require('fs'),
    ...
    storage = pkgcloud.providers.joyent.storage;

// BW FIXME var decls unify for style
...
const MAX_UPLOAD = 50 * 1024 * 1024 * 1024; // BW FIXME: should we have a limit here?
...
/*
  moptions = {
    account: options.mantaUser,
    keyId: options.mantaKeyId,
    url: options.mantaUrl,
    copies: 2,
    quiet: true,
    path: urlJoin (options.username, 'stor', container, options.remote),
    log: createLogger('pkgcloud')
  };
*/
//  client = manta.createBinClient(moptions);



  if (!this.jclient) {
    console.log ('jclient not initialized, punting');
  } else {
    var opts = {
      headers: {
    ...
    return this.jclient.createWriteStream('~~/stor/testjclientstream.misc', opts);
  }
};
